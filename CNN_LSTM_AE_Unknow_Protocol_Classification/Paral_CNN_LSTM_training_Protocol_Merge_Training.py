...
ConvAE.save_weights(save_dir + '/'+'save_weight_CNN_LSTM_Merge.ckpt')
...
#encoder = Model(inputs=img_inputs, outputs=encode_output, name='LSTMAE_Encoder')
...
print('Cluster latent with kmeans')
cluster_latent(encoded_img, test_y, n_classes=10)
print('Cluster latent with spectral')
cluster_latent_spectral(encoded_img, test_y, n_classes=10)

print('cluster the raw protocol data with Kmeans')
...
cluster_latent(test_x, test_y, n_classes=9)

print('cluster the raw protocol data with spectral')
cluster_latent_spectral(test_x, test_y, n_classes=10)
