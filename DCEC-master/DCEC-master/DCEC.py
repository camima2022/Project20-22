...
class ClusteringLayer(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.clusters = self.add_weight(name='clusters', shape=(self.n_clusters, input_dim), initializer='glorot_uniform',trainable=True )
...
class DCEC(object):
    ...
    def pretrain(self, x, y=None, batch_size=256, epochs=20, optimizer='adam', save_dir='results/temp'):
        print('...Pretraining...')
        ...
        if y is not None:
            class PrintACC(callbacks.Callback):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    if int(epochs / 10) != 0 and epoch % int(epochs / (epochs/5)) != 0:
                        return
                    feature_model = Model(self.model.input,
                                          self.model.get_layer(name='H1').output)
        ...
        # begin training
        t0 = time()
        self.cae.fit(x, x, batch_size=batch_size, epochs=epochs, callbacks=cb)
    ...
    def fit(self, x, y=None, batch_size=144, maxiter=2e4, tol=1e-3,
        ...
        if not self.pretrained and cae_weights is None:
            ...
            self.pretrain(x, y, batch_size, save_dir=save_dir)
            self.pretrained = True
...
# x, y = load_mnist()
# n_clusters = 10
# dcec1 = DCEC(input_shape=x.shape[1:], filters=[32, 64, 128, 10], n_clusters=n_clusters)
...
if __name__ == "__main__":
    ...
    save_dir = '/home/hbb/pythonProject/DCEC-master/temp'
    gamma = 0.1
    tol = 0.001
    maxiter = 2e7
    update_interval = 140
    ...
    # load dataset
    from datasets import load_mnist, load_usps,load_dataset_ids17
    while True:
        ...
        dcec.fit(x, y=y, tol=tol, maxiter=maxiter,
        ...
        if metrics.nmi(y, y_pred)>0.82 and metrics.ari(y, y_pred)>0.76:
            break