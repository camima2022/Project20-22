...
def CLSTM_AE():
    ...
    Encoder = Model(inputs=encode_inputs, outputs=encode_outputs, name='Encoder')
    return autoEncoder, Encoder
def CLSTM_Attention_AE_v1():
    encode_inputs = Input(shape=(length, 1))
    e = Conv1D(filters=128, kernel_size=5, strides=4, padding='same', activation='relu')(encode_inputs)
    e = BatchNormalization(momentum=0.9)(e)
    e = Conv1D(filters=128, kernel_size=1, strides=1, padding='valid', activation='relu')(e)
    e = BatchNormalization(momentum=0.9)(e)
    ...
    d_img_output = Activation('relu', name='d_relu3')(d)
    d_cp_output = BatchNormalization(momentum=0.9)(d_img_output)
    d = Conv1DTranspose(filters=64, kernel_size=1, strides=1, padding='valid', activation='relu')(d_cp_output)
    d = BatchNormalization(momentum=0.9)(d)
    d = Conv1DTranspose(filters=64, kernel_size=5, strides=4, padding='same', activation='relu')(d)
    d = BatchNormalization(momentum=0.9)(d)
    d = Conv1DTranspose(filters=128, kernel_size=1, strides=1, padding='valid', activation='relu')(d)
    d = BatchNormalization(momentum=0.9)(d)
    d = Conv1DTranspose(filters=128, kernel_size=5, strides=4, padding='same', activation='relu')(d)
    decode_output = TimeDistributed(Dense(1))(d)
...
def NIN_LSTM_Attention_AE():
    ...
    e = BatchNormalization(momentum=0.9)(e)
    e = Conv1D(filters=64, kernel_size=1, strides=1, padding='valid', activation='relu')(e)
    cp_input = BatchNormalization(momentum=0.9, name='cp_input')(e)
    avgpool = GlobalAveragePooling1D(name='channel_avgpool')(cp_input)
    maxpool = GlobalMaxPool1D(name='channel_maxpool')(cp_input)
...
#Channel Attention, Spatial Attention, Self Attention + NIN + BLSTM
def NIN_LSTM_CSS_Attention_AE():
    encode_inputs = Input(shape=(length, 1))
    e = Conv1D(filters=128, kernel_size=5, strides=4, padding='same', activation='relu')(encode_inputs)
    e = BatchNormalization(momentum=0.9)(e)
    e = Conv1D(filters=64, kernel_size=1, strides=1, padding='valid', activation='relu')(e)
    ...
    cp_output = BatchNormalization(momentum=0.9)(img_output)
    e = Bidirectional(LSTM(units=32, return_sequences=True, recurrent_dropout=0.2, dropout=0.1), name='LSTM1')(cp_output)
    e = Attention(name='encoder_attention')([e, e])
    e = BatchNormalization(momentum=0.9)(e)
    encode_outputs = Bidirectional(LSTM(units=2, return_sequences=False, recurrent_dropout=0.2, dropout=0.1))(e)
    ...
    d = Bidirectional(LSTM(units=2, return_sequences=True, recurrent_dropout=0.2, dropout=0.1))(decode_input)
    d = BatchNormalization(momentum=0.9)(d)
    d = Attention(name='decoder_attention')([d, d])
    d_cp_input = Bidirectional(LSTM(units=32, return_sequences=True, recurrent_dropout=0.2, dropout=0.1))(d)
    d_avgpool = GlobalAveragePooling1D(name='d_channel_avgpool')(d_cp_input)
    d_maxpool = GlobalMaxPool1D(name='d_channel_maxpool')(d_cp_input)
...
def DEC_IDS_GCX():
    dims = [500, 500, 2000, 4]
    encode_inputs = Input(shape=(length,))
    e = Dense(units=dims[0], activation='relu')(encode_inputs)
    e = Dense(units=dims[1], activation='relu')(e)
    e = Dense(units=dims[2], activation='relu')(e)
    encode_outputs = Dense(dims[-1], activation='relu')(e)
    decode_inputs = Dense(dims[2], activation='relu')(encode_outputs)
    d = Dense(dims[1], activation='relu')(decode_inputs)
    d = Dense(dims[0], activation='relu')(d)
    decode_outputs = Dense(units=length, activation='relu')(d)
    autoEncoder = Model(inputs=encode_inputs, outputs=decode_outputs, name='AE')
    Encoder = Model(inputs=encode_inputs, outputs=encode_outputs, name='Encoder')
    return autoEncoder, Encoder
...
def NIN_LSTM_AE():
    ...
    e = Bidirectional(LSTM(units=32, return_sequences=True, recurrent_dropout=0.2, dropout=0.1), name='LSTM1')(e)
    e = BatchNormalization(momentum=0.9)(e)
    encode_outputs = Bidirectional(LSTM(units=2, return_sequences=False, recurrent_dropout=0.2, dropout=0.1))(e)
    decode_input = RepeatVector(8)(encode_outputs)
    d = Bidirectional(LSTM(units=2, return_sequences=True, recurrent_dropout=0.2, dropout=0.1))(decode_input)
    d = BatchNormalization(momentum=0.9)(d)
    d_cp_input = Bidirectional(LSTM(units=32, return_sequences=True, recurrent_dropout=0.2, dropout=0.1))(d)
    d_cp_output = BatchNormalization(momentum=0.9)(d_cp_input)
...
class DEC(object):
    def __init__(self, dims, n_clusters=num_clusters, alpha=1.0):
        ...
        self.n_clusters = n_clusters
        self.alpha = alpha
        self.autoencoder, self.encoder = NIN_LSTM_Attention_AE()
            ...
            class PrintACC(callbacks.Callback):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    ...
                    print(' ' * 8 + '|==>  acc: %.4f,  nmi: %.4f,  ari: %.4f  <==|'
                          % (metrics.acc(self.y, y_pred, num_cluster=num_clusters), metrics.nmi(self.y, y_pred), metrics.ari(self.y, y_pred)))
