...
class DEC(object):
    def __init__(self, dims, n_clusters=5, alpha=1.0):
        ...
        if y is not None:
            class PrintACC(callbacks.Callback):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    ...
                    km = KMeans(n_clusters=5, n_init=20, n_jobs=4)
                    y_pred = km.fit_predict(features)
                    print(' ' * 8 + '|==>  acc: %.4f,  nmi: %.4f  <==|'
                          % (metrics.acc(self.y, y_pred, num_cluster=5), metrics.nmi(self.y, y_pred)))
    ...
    def fit(self, x, y=None, maxiter=2e4, batch_size=256, tol=1e-3,
        ...
        for ite in range(int(maxiter)):
            if ite % update_interval == 0:
                ...
                y_pred = q.argmax(1)
                if y is not None:
                    acc = np.round(metrics.acc(y, y_pred, num_cluster=5), 5)
...
def load_dataset():
    ...
    path = '/home/hbb/pythonProject/CNN_LSTM_AE_Unknow_Protocol_Classification/ISCX2012_1000_version_5_class/'
    class_list = ['ftp', 'http', 'imap', 'pop3', 'smtp']
    data, label = DataCollection.DataPayloadCollection_5class(path, class_list)
...
if __name__ == "__main__":
    ...
    print('acc:', metrics.acc(y, y_pred, num_cluster=10))
