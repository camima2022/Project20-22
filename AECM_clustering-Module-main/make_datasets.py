...
def load_dataset_ISCX12():
    ...
    path = '/home/hbb/pythonProject/data/ISCX02/'
    class_list = ['bittorrent', 'smb', 'http', 'pop']
    ...
    x = CountVectorizer(dtype=np.float32, max_features=2000).fit_transform(x)
    x = TfidfTransformer(norm='l2', sublinear_tf=True).fit_transform(x).toarray()
    s = (train_x.shape, test_x.shape)


    np.savez('data/ISCX02.npz',x=x,y=y,s=s)
    # print(x.shape)
    ...
    try:
        os.mkdir('../AECM_clustering-Module-main/ISCX02')
        os.mkdir('../AECM_clustering-Module-main/ISCX02/save')
    except FileExistsError:
        pass

def load_dataset_dns():
    import DataCollection_UN15
    path = '/home/hbb/pythonProject/data/temp/'
    class_list = ['dns', 'imap', 'http', 'pop']
    ...
    x = x.reshape((x.shape[0], -1))
    s = (train_x.shape, test_x.shape)
    np.savez('data/DNS.npz',x=x,y=y,s=s)
    # print(x.shape)
    # print('dataset shapes', x.shape)
    try:
        os.mkdir('../data/DNS')
        os.mkdir('../data/DNS/save')
    except FileExistsError:
        pass

def load_dataset_un15():
    import DataCollection_UN15
    path = '/home/hbb/pythonProject/data/UN15/'
    ...
    x = x.reshape((x.shape[0], -1))
    s = (train_x.shape, test_x.shape)
    np.savez('data/UN15.npz',x=x,y=y,s=s)
    # print(x.shape)
    # print('dataset shapes', x.shape)
    try:
        os.mkdir('../data/UN15')
        os.mkdir('../data/UN15/save')
    except FileExistsError:
        pass
...
def load_dataset_ids17():
    ...
    x = x.reshape((x.shape[0], -1))
    s = (train_x.shape, test_x.shape)
    np.savez('data/IDS17V3.npz', x=x, y=y, s=s)
    # print(x.shape)
    # print('dataset shapes', x.shape)
    try:
        os.mkdir('IDS17V3')
        os.mkdir('IDS17V3/save')
    except FileExistsError:
        pass

def load_dataset_ids17v4():
    import DataCollection_IDS
    ...
    x = x.reshape((x.shape[0], -1))
    s = (train_x.shape, test_x.shape)
    np.savez('data/IDS17V4.npz', x=x, y=y, s=s)
    # print(x.shape)
    # print('dataset shapes', x.shape)
    try:
        os.mkdir('IDS17V4')
        os.mkdir('IDS17V4/save')
    except FileExistsError:
        pass
...
if __name__ == "__main__":
    ...
    for name,make in [
        ...
        ('ISCX02', load_dataset_ISCX12),
        # ('dns', load_dataset_dns),\
        # ('un15', load_dataset_un15),\
       # ('IDS17', load_dataset_ids17),
       #  ('IDS17v4', load_dataset_ids17v4),
       # ('MNIST',make_mnist), \
#        ('FMNIST',make_fmnist), \
#        ('USPS',make_usps), \
#         ('R10K',make_reuters), \
