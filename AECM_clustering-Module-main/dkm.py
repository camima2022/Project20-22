...
LOAD = np.load('data/'+dataset.upper()+'.npz',allow_pickle=True)
data = LOAD['x']
target = LOAD['y']
del LOAD
print("Dataset",dataset.upper(),"loaded...")
...
hidden_2_size = 500
hidden_3_size = 2000
embedding_size = int(AECM_UNIF[dataset.upper() ]['OUT']) # n_clusters #
...
n_pretrain_epochs = p_epochs
n_finetuning_epochs = f_epochs
...
batch_size = batch_size # Size of the mini-batches used in the stochastic optimizer
n_batches = int(math.ceil(n_samples / batch_size)) # Number of mini-batches
validation = validation # Specify if data should be split into validation and test sets
pretrain = pretrain # Specify if DKM's autoencoder should be pretrained
annealing = annealing # Specify if annealing should be used
seeded = seeded # Specify if runs are seeded

if lambda_ == -1:
    lambda_ = DIC_LBD[dataset.upper()][ 1 if pretrain else 0 ] # args.lambda_ # MNIST .01
else:
    lambda_ = lambda_ # MNIST .01

print("Hyperparameters...")
print("lambda =", lambda_)

if pretrain:
    AE = np.load(dataset.upper()+'/save/save-aecm-rand.npz', allow_pickle=True)['wgt']
    print(AE)

FNAME = dataset.upper()+'/save/save-dkm-'+('rand' if annealing else ( 'pre' if pretrain else 'unif') )+'-lbd='+str(lambda_)
print(FNAME)
...
if cpu:
    os.environ['CUDA_VISIBLE_DEVICES'] = '0' # Run on CPU instead of GPU if batch_size is small
...
for run in range(n_runs):
    ...
    if pretrain:
        ...
        embeddings = tf.print((cg.embedding,), feed_dict={cg.input: data})[0].squeeze()
        ...
        tf.print(tf.assign(cg.cluster_rep, kmeans_model.cluster_centers_))

    # Train the full DKM model
    if (len(alphas) > 0):
        print("Starting DKM training", dataset.upper(), ('Pretrain' if pretrain else ( 'Annealing' if  annealing else 'Uniform')))
        ...
        for _ in range(n_finetuning_epochs):
            ...
            for _ in range(n_batches):
                ...
                _, loss_, stack_dist_, cluster_rep_, ae_loss_, kmeans_loss_ =\
                    tf.print((cg.train_op, cg.loss, cg.stack_dist, cg.cluster_rep, cg.ae_loss, cg.kmeans_loss),
...
#                    break
    ...
    print("Running k-means on the learned embeddings...")
    embeddings = print((cg.embedding,), feed_dict={cg.input: data})[0].squeeze()
    kmeans_model = KMeans(n_clusters=n_clusters, init="k-means++").fit(embeddings)
    ...
    if dataset.upper() == 'MNIST':
        cluster_rep_ = print((cg.cluster_rep, ), feed_dict={})[0]
        list_mu.append( cluster_rep_ )

        cluster_rep_out = print((cg.output, ), feed_dict={cg.embedding: cluster_rep_})
