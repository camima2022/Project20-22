...
path = '/CNN_LSTM_AE_Unknow_Protocol_Classification/ISCX2012_1000_version_5_class/'

class_list = ['dns', 'ftp', 'http', 'imap', 'pop3', 'smtp']
nb_classes = 6
length = 784

data, label = DataCollection.DataPayloadCollection_6class(path, class_list)

train_x, test_x, train_y, test_y = train_test_split(data, label, test_size= 0.2, random_state=1234, shuffle=True)
...
def AE():
    ...
    x = BatchNormalization(momentum=0.9)(x)
    encode_outputs = Bidirectional(LSTM(units=32, return_sequences=False, recurrent_dropout=0.2, dropout=0.1))(x)

    # decode
    decode_input = RepeatVector(98)(encode_outputs)
    d = Bidirectional(LSTM(units=32, return_sequences=True, recurrent_dropout=0.2, dropout=0.1))(decode_input)
...
def cluster_latent_spectral(latent, labels, n_classes=9):
    ...
    lsshc = LSC_DensityCanopyKmeans.LSSHC(mode_framework='eigen_trick', mode_Z_construction='knn', mode_Z_construction_knn='random',
                  mode_sampling_schema='HSV', mode_sampling_approach='probability', m_hyperedge=200, l_hyperedge=80,
                  knn_s=3, k=n_classes)
...
history = ConvAE.fit(train_x, train_x, batch_size=64, epochs = 10, verbose = 2, validation_data=(test_x, test_x))
ConvAE.save_weights(save_dir + '/'+'save_weight_6_class.ckpt')
Encoder.save_weights(save_dir + '/' + 'save_encoder_weight_6_class.ckpt')
...
fig.savefig(result_path+'/pretraining_our_6_class.png')
fig1 = plt.figure()
...
fig1.savefig(result_path+'/pretraining_loss_6_class.png')


encoded_img = Encoder.predict(test_x)

print(encoded_img.shape)
cluster_viz(encoded_img, test_y, save_dir, n_classes=6, fig_name='tsne_viz_6_class.png')
test_x = test_x.reshape(test_x.shape[0], 784)
cluster_viz(test_x, test_y, save_dir, n_classes=6, fig_name='tsne_viz_6_class_raw_data.png')


from sklearn import preprocessing
min_max_scaler = preprocessing.MinMaxScaler()
latent = min_max_scaler.fit_transform(encoded_img)

cluster_latent_spectral(latent, test_y, n_classes=6)
cluster_latent(latent, test_y, n_classes=6)