...
def change(data_list):
    ...
    return change_data
def readPcap_with_ip_tcp_layer(file):
    payload_data = []
    src = []
    dst = []
    sport = []
...
def readPcap_pure_application_layer(file):
    payload_data = []
    src = []
    dst = []
    sport = []
...
def readPcap_smtp(file):  # smtp协议的应用层载荷是从第67位开始的，和其他的基于TCP的应用层协议不一样
    ...
    return payload_data, session
def readPcap_smtp_un15(file):  # smtp协议的应用层载荷是从第67位开始的，和其他的基于TCP的应用层协议不一样
    payload_data = []
    src = []
    dst = []
    sport = []
    ...
    with open(file, 'rb') as f:
        ...
        for ts, packet in packets:
            ...
            if transfer_flag == int(6):
                tcp_layer_data = packet[38:54]
                raw_data = packet[68:length]  # smtp协议的应用层载荷是从第67位开始的，和其他的基于TCP的应用层协议不一样
        ...
        for i in range(len(class_list)):
            ...
            if class_list[i] in cwd:
                if class_list[i] == 'smtp':
                    ...
                    data, session = readPcap_pure_application_layer(file=cwd)
...
def DataPayloadCollection_5class(path, class_list):
    payload_data = []
    labels = []
    for name in os.listdir(path):
        cwd = path + name
    ...
    label_numpy = np.asarray(labels)
    return data_numpy, label_numpy
def DataPayloadCollection_6class(path, class_list):
    payload_data = []
    labels = []
    for name in os.listdir(path):
        cwd = path + name
    ...
    print('0-dns:{}\n1-ftp:{}\n2-http:{}\n3-imap:{}\n4-pop3:{}\n5-smtp:{}\n'.format(
            labels.count(0),
            labels.count(1), labels.count(2), labels.count(3), labels.count(4), labels.count(5)))
...
def DataPayloadCollection_8class(path, class_list):
    payload_data = []
    labels = []
    for name in os.listdir(path):
        cwd = path + name
    ...
    label_numpy = np.asarray(labels)
    return data_numpy, label_numpy
def DataPayloadCollection_ids17(path, class_list):
    payload_data = []
    labels = []
    for name in os.listdir(path):
        cwd = path + name
